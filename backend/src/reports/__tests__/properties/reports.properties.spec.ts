...
describe('Reports Module - Property-Based Tests', () => {
  ...
  describe('Property 8: Balance Calculation Accuracy', () => {
    it('should calculate balance as sum of paid payments with 2 decimal precision', async () => {
      await fc.assert(
        fc.asyncProperty(
          uuidArbitrary(),
          fc.array(paymentArbitrary(), { minLength: 0, maxLength: 20 }),
          async (buildingId, payments) => {
            // Calculate expected values (income only — expense tracking not in schema)
