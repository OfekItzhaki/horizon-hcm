  ...
  implements ICommandHandler<UpdatePreferencesCommand>
{
  ...
  async execute(command: UpdatePreferencesCommand) {
    ...
    if (paymentReminders !== undefined)
      updateData.payment_reminders = paymentReminders;
    if (maintenanceAlerts !== undefined)
      updateData.maintenance_alerts = maintenanceAlerts;
    if (meetingNotifications !== undefined)
      updateData.meeting_notifications = meetingNotifications;
    if (generalAnnouncements !== undefined)
      updateData.general_announcements = generalAnnouncements;
